<?php
/**
 * Fixed Import Variable Products Script
 * 
 * This is a corrected version of the import-variable-products.php script
 * that properly handles duplicate variations and prevents them from being imported twice.
 * 
 * Usage: php fixed-import-variable-products.php [--output=file.txt]
 */
...
// Track all imported variation IDs to avoid duplicates
$imported_variation_ids = [];
        ...
        if (isset($wc_product->type) && $wc_product->type === 'variable') {
            ...
            if ($parent_id) {
                ...
                if (!empty($variations)) {
                    ...
                    // Track variations processed for this product
                    $processed_variation_ids = [];
                    
                    foreach ($variations as $variation) {
                        // Skip if already processed (avoid duplicates)
                        if (in_array($variation->id, $processed_variation_ids) || 
                            in_array($variation->id, $imported_variation_ids)) {
                        ...
                        $imported_variation_ids[] = $variation->id;
                        
                        // Check if variation exists already
                        $existing_variation = $product_manager->getByProductId($variation->id);
                        if ($existing_variation) {
                            echo "    - Variation already exists in database, skipping: " . $variation->id . "\n";
                            continue;
                        }
                ...
                fwrite($log_file, "FAILED: " . $wc_product->name . " (ID: " . $wc_product->id . ")\n");
            }
        }

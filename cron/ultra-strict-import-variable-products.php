<?php
/**
 * Ultra-Strict Variable Products Import Script
 * 
 * This script uses a comprehensive approach to ensure absolutely no duplicate variations
 * are imported. It addresses the persistent issue where 634 actual variations were importing as 785.
 * 
 * Key improvements:
 * - Triple verification before import
 * - Global tracking of variations across all WooCommerce
 * - Better pagination handling
 * - Multiple data validation steps
 * - Detailed logging with exact counts
 * - Database verification before each import
 * 
 * Usage: php ultra-strict-import-variable-products.php [--output=file.txt] [--debug] [--dry-run]
 */
...
$options = getopt('', ['output:', 'debug', 'dry-run']);
$output_file = isset($options['output']) ? $options['output'] : 'ultra-strict-variable-products-import.log';
$debug_mode = isset($options['debug']);
$dry_run = isset($options['dry-run']);

echo "Starting ultra-strict variable products import...\n";
echo "Debug mode: " . ($debug_mode ? "ON" : "OFF") . "\n";
echo "Dry run mode: " . ($dry_run ? "ON" : "OFF") . "\n";

// Open log file
$log_file = fopen($output_file, 'w');
fwrite($log_file, "# Ultra-Strict Variable Products Import Log\n");
...
// Helper function to get all variations for a product with retries and validation
function get_all_verified_variations($product_id, $product_name, $woocommerce, $debug_mode, $log_file) {
    $verified_variations = [];
    $seen_variation_ids = [];
    $duplicates_found = 0;
    $max_retries = 3;
    ...
    for ($retry = 0; $retry < $max_retries; $retry++) {
        try {
            $variations = $woocommerce->getProductVariations($product_id);
    ...
    // If we got a large number, do a secondary verification with pagination
    if (count($verified_variations) >= 90) {
        log_message("    Large number of variations detected (" . count($verified_variations) . "), verifying with pagination...", $log_file);
        
        $paginated_variations = [];
        $paginated_seen_ids = [];
        $per_page = 20;
        $page = 1;
        
        // Use manual pagination to verify
        do {
            try {
                // Manually construct the request with pagination params
                ...
                $page_variations = $woocommerce->makeRequest($endpoint, 'GET', $params);
                
                if (empty($page_variations)) {
                    log_message("    No variations found on page $page", $log_file);
                    break;
                }
                
                log_message("    Found " . count($page_variations) . " variations on page $page", $log_file);
                
                // Process variations from this page
                foreach ($page_variations as $variation) {
                    if (!isset($variation->id)) continue;
                    
                    if (!isset($paginated_seen_ids[$variation->id])) {
                        $paginated_seen_ids[$variation->id] = true;
                        $paginated_variations[] = $variation;
                    } else {
                        $duplicates_found++;
                        if ($debug_mode) {
                            log_message("    DUPLICATE DETECTED in pagination: Variation ID {$variation->id}", $log_file);
                        }
                    }
                }
                ...
                log_message("    ERROR during paginated verification: " . $e->getMessage(), $log_file);
                break;
            }
            
        } while (!empty($page_variations) && count($page_variations) == $per_page);
        
        // Compare the counts
        log_message("    Verification complete: " . count($verified_variations) . " from standard fetch vs " . 
                   count($paginated_variations) . " from paginated fetch", $log_file);
        
        // If paginated result has more or a different number, use that instead
        if (count($paginated_variations) != count($verified_variations)) {
            log_message("    USING PAGINATED RESULTS due to count discrepancy", $log_file);
            return [
                'variations' => $paginated_variations,
...
foreach ($all_parent_products as $parent_id => $parent_product) {
    $result = get_all_verified_variations($parent_id, $parent_product->name, $woocommerce, $debug_mode, $log_file);
...
$details = "Imported with ultra-strict validation";
